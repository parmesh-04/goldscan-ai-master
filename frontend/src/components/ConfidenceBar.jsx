...
export default function ConfidenceBar({ value = 0, label, className = '', performed = true }) {
  // Animate from 0 → value on mount so the fill transition is actually visible.
  // Without this, the bar jumps straight to its target width on first render
  // because there is no prior state for the CSS transition to animate from.
  const [displayValue, setDisplayValue] = useState(0);
  useEffect(() => {
    const t = requestAnimationFrame(() => setDisplayValue(value ?? 0));
